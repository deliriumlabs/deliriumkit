...
function popUpSubMenu(menuItemObj) { // Private method
  ...
  if (parentMenuObj.style.position == "fixed") {
    ...
    menuObj.style.top = test_top(getSubMenuTopPos(menuObj, y, menuItemObj.offsetHeight)) + px;
    ...
    if (parentMenuObj.mode == "static" && !ie50) {
      ...
      menuObj.style.top = test_top((getSubMenuTopPos(menuObj, y, menuItemObj.offsetHeight) + getScrollTop())) + px;
    }
    else {
...
function popUpMainMenu(menuObj, e) { // Private method
  ...
  menuObj.style.top = test_top(getMainMenuTopPos(menuObj, getY(e)) + getScrollTop()) + px;
...
function scrollHandler() { // Private method
  for (var i = 1; i <= menuCount; i++) {
      ...
      if (ie && menuObj.mode == "fixed") {
        ...
        menuObj.style.top = test_top(menuObj.initialTop + getScrollTop()) + px;
      }
  }
...
function showMenuItemIcon() { // Public method
  ...
  iconElm.style.top = test_top(Math.floor((this.offsetHeight - height) / 2)) + px;
  if (ie) {
...
function setSubMenu(menuObj) { // Public method
  ...
  arrowElm.style.top = test_top(Math.floor((this.offsetHeight - height) / 2)) + px;
  ...
  menuObj.menuObj.style.zIndex = 100+(this.parent.menuObj.level + 1);
...
function jsDOMenu() { // Public method
  ...
  this.setY = function(y) { // Public method
    ...
    this.menuObj.style.top = test_top(y) + px;
  };
